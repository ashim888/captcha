...
class StatusCheckerUI(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent)
        self.isRunning = False
        font=wx.SystemSettings_GetFont(wx.SYS_SYSTEM_FONT)
        font.SetPointSize(20)
        lblTitle = wx.StaticText(self,label="Site Status Checker", pos=(20,35))
        lblTitle.SetFont(font)
        lblUrl = wx.StaticText(self,label="Url File",size=(60,30))
        lblUrl.ToolTipString = 'File that contains urls'
        self.urlFile_txt=wx.TextCtrl(self)
        self.urlFile_txt.ToolTipString = 'File that contains urls'
        self.btnBrowseUrl = wx.Button(self,label="Browse")
        self.btnBrowseUrl.ToolTipString = 'File that contains urls'
    ...
    def start(self,event):
        ...
        self.datacontext = statuschecker.SiteStatusChecker(urlFile,proxyFile,self.evt,self.thread_slider.GetValue())
